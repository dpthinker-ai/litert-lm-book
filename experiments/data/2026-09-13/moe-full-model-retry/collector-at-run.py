#!/usr/bin/env python3
"""Repeated full MoE generation with raw C stream events and runtime metrics.

New collector derived from the immutable moe_full_model_check.py. Each process
...
def worker(args):
    ...
    result=dict(phase='engine_creation',engine_created=False,generation_completed=False,
                prompt=args.prompt,generations=[])
        ...
        with lm.Engine(str(args.model),backend=lm.Backend.GPU(),max_num_tokens=args.context,
            ...
            for index in range(args.repeats):
                ...
                with engine.create_conversation(
                    ...
                    item=dict(index=index,events=[],stream_final=False,stream_error=None)
...
def main():
    ...
    report=dict(started_at_utc=datetime.now(timezone.utc).isoformat(),python=sys.version,
        ...
        repetitions_in_engine=args.repeats,benchmark_enabled=True,
        ...
        limitations=['Prebuilt package; not a local build of frozen release source.',
            ...
            'Client callback timestamps include queueing; callback count is not a token count.',
