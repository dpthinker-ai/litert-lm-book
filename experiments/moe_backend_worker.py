#!/usr/bin/env python3
"""Small public-C-ABI probe for CPU, automatic GPU, or explicit WebGPU.
...
def worker(directory, libpath, backend, precision):
    ...
    try:
        ...
        call('LiteRtRunCompiledModel',compiled,0,input_count,(P*input_count)(*bufs[:input_count]),1,(P*1)(bufs[-1]))
        ...
        call('LiteRtUnlockTensorBuffer',bufs[-1]);np.save(directory/'actual.npy',output)
...
if __name__=='__main__':
    ...
    raise SystemExit(worker(args.directory.resolve(),args.library.resolve(),args.backend,args.precision))
