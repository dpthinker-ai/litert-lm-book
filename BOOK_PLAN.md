...
> 全稿已成：前言、11 章、尾声、附录 A-E，37 张 SVG。已发布基线为 Tag v0.1.0；2026-09-05 的技术纠错、实测补充与全书复核已提交到 main，尚未打新 Tag。开放项见下文“当前实测与开放项”。
> 修订记录以 git 提交历史为准；本文件只保留全书定位、目录与开放项。
> 初稿期的详细规划（章卡、路线图、决策记录）见 git 历史与 `archive/`。

## 定位

以 Google 的端侧 LLM 运行时 **LiteRT-LM**（`google-ai-edge/LiteRT-LM` @ **v0.13.1**，已冻结）为主要分析对象，系统回答：大语言模型如何在手机、手表和浏览器等受限设备上运行。视角是**实现与权衡**，不是 API 教程，也不是优化技术综述；非官方出版物。
