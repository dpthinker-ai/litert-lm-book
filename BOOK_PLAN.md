...
> 状态：**v4.0 — 完整初稿**（2026-07-05）：**前言 + 11 章 + 尾声 + 附录 A-F 全部成稿**（正文约 10 万字、12 张 SVG 图、12 份 review.md，代码引用逐条对 v0.13.1 核验）。
...
> **v4.1（2026-07-05）：基准数据集已采集并回填全书**（Gemma 4 E4B，30 次矩阵 + 强制 MTP 验证；附录 D 含结果表与四条对账解读，其中 MTP "未复现 3x" 如实报告）。
> **v4.2（2026-07-16）：全书评审修订**（评审报告 `review-report.html`）。修复 8 处事实硬伤：第 9 章 auto 探测链路按 v0.13.1 实况改写（auto 实为关）、第 6 章克隆装载链路按死代码实况重写（move 零拷贝）、第 4 章填充算例复算、第 2 章吞吐数量级（3-20 倍）、第 1 章 KV 分配分静态/动态、停止符口径统一为 `<turn|>`、logits 回搬统一为 FP32/1 MiB、第 10 章 52 MiB 与占位符清理；交叉引用专项约 15 处；留痕注释、单位（112 MiB / 875 MiB）、lint 复归 0。
> **v4.3（2026-07-17）：扩容工程启动（目标 ~250 页）**。学习网站借鉴落地：图 2-3 端到端数据流（SVG 重制为书版）、术语表 4 增 1 修、附录 B 读码路径。Tier 1 补欠账：7 张权衡表与 6 张缺口图全部补齐；ch01 动机与小结、ch03 EngineFactory、ch07 weight cache、ch11 Copybara 补齐。Tier 2：附录 G（二十问速查）、附录 H（issue 案例集）新建，附录 D 扩原始记录，尾声集成路径扩写。Tier 3 深潜与实测：ch04 隐性 CPU 开销小节、ch08 NPU 缓冲共享、**接受率实证归因**（免改码路径：SDK VERBOSE 读 drafter 析构计数，故事 α≈0.26 / 代码 α≈0.995）、**max-num-tokens 扫描**（8192 比 1024 慢约 37%，1024 档工单打满直接越界）、**约束解码开/关实测**（简单场景 8/8 无差异）。仍开放（附录 D 对账清单）：int4 vs int8、并行加载开关、CPU 线程扫描、Python/C++ 一致性、多模态端到端、双 tokenizer 对比。
...
> 全部奠基决策拍板完毕；"一个 token 的一生"意象留给前言与第二篇部题。
...
### 第二篇 · 一个 token 的一生（3 章，约 70 页，全书脊柱）
...
全书核心承诺全部兑现；第四篇与附录是完整版增量。这是防止烂尾的安全着陆点。
