...
> **v4.2（2026-07-16）：全书评审修订**（评审报告 `review-report.html`）。修复 8 处事实硬伤：第 9 章 auto 探测链路按 v0.13.1 实况改写（auto 实为关）、第 6 章克隆装载链路按死代码实况重写（move 零拷贝）、第 4 章填充算例复算、第 2 章吞吐数量级（3-20 倍）、第 1 章 KV 分配分静态/动态、停止符口径统一为 `<turn|>`、logits 回搬统一为 FP32/1 MiB、第 10 章 52 MiB 与占位符清理；交叉引用专项约 15 处；留痕注释、单位（112 MiB / 875 MiB）、lint 复归 0。仍开放（评审报告 P2）：章卡承诺的 7 张权衡表与图 5-2/6-3/7-2/8-2/9-2/10-2、约 10 个未做实验、ch01 收益动机与小结、附录 D 对账清单两个 ⬜。
