...
上下文扩展测试在容量 512、1024、2048 下分别运行短输入与材料输入，共 6 进程×3 新会话，18 次均正常结束。实际 prefill 分别为短输入 23、材料输入 397/909/1933，每次 runtime decode 计数为 20；九次材料回答均包含正确识别码。压力采样有 12 次 warning，其余为 normal，未触发保护停止。容量与实际输入、首次与复用指标分别记录，见附录 D 第二十七节。

尚未完成的实验，正文均已按"来源于代码分析/无实测数据"如实标注：
- MoE 完整模型导出、系统性质量验证；自定义量化 GPU 算子与 NPU 支持、内存与持续性能测量。公开 Artisan 产物已完成短文本重复生成与 512/1024/2048 容量测试，实际 prefill 测至 1933 token；此前原生与 Web 测试的保护停止记录保留
- 编译缓存开/关的外部墙钟受控对照
