...
| `support/util/io_types.h`、`runtime/conversation/model_data_processor/multimodal_processor_helper.cc` | 输入数据表示与多模态预处理组装 | 第 3、10、11 章 |
...
| `runtime/components/constrained_decoding/` | 约束解码（ProcessLogits / llguidance / LogitMask） | 第 10 章 |
| `runtime/components/tool_use/`（含 `antlr/*.g4`） | 工具调用格式化与解析 | 第 10 章 |
| `runtime/components/lora.*`、`lora_manager.*` | LoRA 适配器 | 第 7 章 |
| `support/preprocessor/` | 图像预处理、patchify | 第 10 章 |
...
| `c/engine.{h,cc}`、`c/engine_internal.h`、`c/conversation.{h,cc}`、`c/conversation_internal.h` | C ABI、不透明句柄、结构化流式回调 | 第 11 章 |
