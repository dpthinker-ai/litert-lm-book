...
## 二十七、完整 MoE 的上下文容量与输入长度（2026-09-13）

本组沿用第二十六节的 Apple M5 Pro、24 GiB 内存、macOS 26.5、v0.17.0 预编译包及 Gemma 4 26B-A4B GPU 量化产物，完整模型与动态库哈希相同。实际后端为 Artisan／Metal，计算配置 F16；benchmark 开启，thinking 与投机解码关闭，top-k=1、temperature=0、seed=42。按下表顺序串行运行八个进程，每个进程创建一个引擎、三个新会话，并使用新的输出和缓存目录。操作系统文件页缓存未清除，其他应用负载未隔离。
...
下表保留每个引擎的首次请求；复用值为后两个新会话的中位数，括号为最小值至最大值。首个文本回调与运行时 decode 吞吐的定义沿用第二十六节。材料构造和分词在生成计时之前完成。
...
512／1024 的四组数据归档于 `experiments/data/2026-09-13/moe-context/`；2048 与 4096 各两组数据分别位于同日的 `moe-context-2048/`、`moe-context-4096/`。三个目录分别保存报告、完整流事件、原生日志、压力采样及材料原文与 tokenizer ID。`summary.json` 区分首次与复用请求，`manifest.json` 校验文件；两份采集脚本快照与各报告中记录的脚本哈希一致。复现见附录 C 第十七节。

## 二十八、完整 MoE 的较长输出（2026-09-13）

本组沿用第二十七节的 Apple M5 Pro、24 GiB 内存、macOS 26.5、v0.17.0 预编译包及完整 Gemma 4 26B-A4B GPU 量化文件。实际路径为 Artisan／Metal、F16，模型与动态库哈希未变。容量固定为 4096，benchmark 开启，thinking 与投机解码关闭，top-k=1、temperature=0、seed=42。输出上限 128、256 各使用一个新进程、新缓存目录，每个引擎运行三个新会话，串行执行。
...
下表区分每个引擎的首次请求与后两个新会话。复用值为两次中位数，括号为最小值至最大值；总生成耗时从发起流调用到消费完结束事件，不含引擎创建。首文本与运行时吞吐沿用第二十六节的定义。
...
数据位于 `experiments/data/2026-09-13/moe-long-output/`，含完整响应、流事件、运行时指标、原生日志与压力采样。`summary.json` 保存首次与复用统计，`manifest.json` 校验文件，采集脚本快照与运行报告中记录的脚本哈希一致。复现见附录 C 第十八节。

## 二十九、完整 MoE 的同一会话多轮累积（2026-09-13）

本组使用第二十八节相同的设备、模型、运行时与生成配置，容量 4096，每轮输出上限 64。一个新进程创建一个引擎，依次建立三个独立会话，每个会话连续发送六轮请求。会话之间不传递历史，六轮内部始终使用同一会话对象；不重发拼接后的完整历史，也不清空会话。
...
首文本等待逐轮从各自的流调用开始独立计时，不含约 26036 ms 的引擎创建。运行时名为 TTFT 的字段始终使用该会话首次 prefill 和 decode，六轮内保持首轮值；不能将它当作后续轮次的首文本时延。表中吞吐使用本轮的 `last_decode_tokens_per_second`。原字段仍完整保存，定义沿用第二十六节。
...
数据位于 `experiments/data/2026-09-13/moe-multiturn/`，逐轮保存提示词、预期字段、完整响应、会话前后计数、流事件与运行时指标。`summary.json` 按会话分组，`manifest.json` 校验原始文件和采集脚本快照。复现见附录 C 第十九节。
