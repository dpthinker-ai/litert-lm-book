...
- 缓存：CLI 使用 `--cache disk`，编译产物持久化在模型旁的缓存文件（取值含义见附录 C 第一节）。同一批次里，gpu/256 的第 1 次 Init API 聚合值为 5.29 s，该条件后两次为 1.76、1.75 s；gpu/4096 的第 1 次为 4.77 s。表中 Init 列是各条件 3 次的中位数，不能代表这些首次调用值。运行前的缓存内容与后续命中情况未单独核查，现有记录不能隔离缓存复用和其他首次准备成本。
...
- KV cache 输入 48 个张量，对应 24 组独立 K/V 缓存，dtype 全部为 INT8；20 组 `[1,2,32003,256]`、4 组 `[1,2,32003,512]`（V 侧维度转置存放）。缓存组数不等于模型层数，跨层共享关系见 6.1 节。
...
- `embeddings[1,1,2560]` → model_dimension = 2560；`per_layer_embeddings[1,1,42,256]`；logits `[1,1,262144]`（FLOAT32）→ 词表 262,144 = 2^18；`param_tensor[1,1,1,7]`（单缓冲 KV 路径的位置参数，见第 6 章）。
...
同一提示词（"Write one sentence about the ocean."）在温度 0、相同种子下运行两次，输出逐字一致。温度 1.0 时，默认采样范围下的 seed 1 与 seed 2 生成了相同序列。seed 7 组同时把 top-k 从默认的 40 改为 64，得到不同句子，不能将差异单独归因于 seed；启用随机采样不保证每次输出都不同。复现时须固定运行时版本、模型文件、后端、提示词、温度、top-k、top-p 与缓存模式；2026-08-31 以重装的新版运行时复测同一命令，温度 0 得到另一句，确定性不跨环境成立。实录见 `experiments/data/temperature_test.md`。
...
四组均为同一模型、输入、GPU 主干与 OpenCL sampler，输出哈希一致；四组共 57 次请求的 decode 计数均为 201 token、非空文本回调均为 200 次。关闭内存采集的前后两组对照中，后续请求首段文本时延的中位数分别为 494、504 ms；持续组为 663 ms，但不能把它与 494 ms 简单相减后称为精确的采样开销，各组的温度、热状态与运行历史并不完全相同。关闭采集的两组，加载时长反而更长，说明跨进程结果还受到这组对照未能隔离的条件影响。
...
这些合成模型未经 litert-torch 导出，不含路由器、注意力、KV cache 或生成循环。结果不证明完整模型正确性、跨后端一致性或端侧部署收益。导出与 GPU 补测分别见第十八、十九节。完整 Gemma 4 的 Artisan 运行数据见第二十一至二十五节；本节自定义算子路径对应的完整模型验证及手机运行仍待完成。Android v0.17.0 数据待重跑的状态保持不变。
...
10.6.4 节所述公开模型冻结到仓库提交 `7228819fa9580751b57b41a93ee54d5c08c4e001`。本次容器核查只以 HTTP Range 读取 GPU、Web 两份文件各自的前 32768 字节；没有下载完整权重或运行推理。两次响应均为 HTTP 206，字节范围和总文件大小与发布元数据一致。GPU 完整文件的后续检查单列于第二十一节。
...
归档目录为 `experiments/data/2026-09-13/moe-scale/`。`report.json`、`summary.json` 保存逐例与汇总数据；输入、参考、回读输出、每次检查和原生日志均有哈希。较大的合成模型文件不入库，按种子和生成器重建后可核对哈希。`collector-at-run.py` 保存实际采集脚本；当前入口仅将硬件字段改为实时读取，数值和计时逻辑未变。进程最高 RSS 包括编译、主机权重和运行开销，不能当作 GPU 分配量。实验未隔离其他应用、频率与温度；固定输入、固定路由的重复调用不证明冷专家缓存或持续 decode 的表现。复现见附录 C 第十节。

## 二十一、完整 MoE 的重复生成与短回合测量（2026-09-13）

本节使用固定仓库提交 `7228819fa9580751b57b41a93ee54d5c08c4e001` 的 Gemma 4 26B-A4B GPU 文件。完整大小为 15786524672 字节（约 14.70 GiB），本地 SHA-256 为 `94bbde2453dd9b67c61c16017af331e5841cbbd9edf83bd2f84bc73e2a7cbdb1`，与发布记录一致。下载来源和校验信息见 `experiments/data/2026-09-13/moe-full-model/download.json`；模型与缓存不入库。

本组使用 Apple M5 Pro、24 GiB 内存、macOS 26.5 和 v0.17.0 预编译包。没有重新导出或量化模型。日志确认 Artisan／Metal、F16；thinking 与投机解码关闭，top-k=1、temperature=0、seed=42。每次启动前保存内存与 swap 状态，但没有隔离全部后台负载。

采集器开启 benchmark，记录流式回调与运行时计数。英文组有三个独立进程，每个进程创建一个引擎，并依次创建三个新会话；中文组有一个进程、三个新会话。每个进程使用新的缓存目录，未清除操作系统文件页缓存。同一进程的后两个会话复用已准备的引擎，不能视为独立的首次启动。
...
首个文本回调时间从请求准备阶段开始，早于回调对象构造和消息序列化；终点是 Python C 回调中读取到首个非空文本的时刻。时间戳在进入消费队列之前记录，排除队列驻留时间；回调调度、GIL 等待及采集开销没有扣除。该时间不包含引擎创建，也不是每 token 的计算时延。四个进程的主机侧引擎创建阶段为 20900–27936 ms，包含创建前的记录对象准备与一次写盘，不能称为严格的 Engine 调用耗时。它与首文本等待分别记录；没有单独测量的记录开销不作扣除。
...
归档目录为 `experiments/data/2026-09-13/moe-full-model-retry/`。`summary.json` 保存逐轮值及汇总，`manifest.json` 校验原始记录；`collector-at-run.py` 对应正式运行时的脚本哈希。当前脚本仅修正了元数据中对消费队列驻留时间的描述，采集和计算逻辑未变。复现见附录 C 第十一节。

## 二十二、完整 MoE 的上下文容量与输入长度（2026-09-13）

本组沿用第二十一节的 Apple M5 Pro、24 GiB 内存、macOS 26.5、v0.17.0 预编译包及 Gemma 4 26B-A4B GPU 量化产物，完整模型与动态库哈希相同。实际后端为 Artisan／Metal，计算配置 F16；benchmark 开启，thinking 与投机解码关闭，top-k=1、temperature=0、seed=42。按下表顺序串行运行八个进程，每个进程创建一个引擎、三个新会话，并使用新的输出和缓存目录。操作系统文件页缓存未清除，其他应用负载未隔离。

短输入仍要求用一句话说明 RAM 保存什么。材料输入先给出识别码 `AMBER-42`，随后重复带编号的 RAM 说明，末尾要求复述识别码并说明 RAM 的用途。脚本使用模型 tokenizer 控制原始文本预算；对话格式由运行时添加。下表中 prefill 计数来自运行时，输出上限与实际 decode 计数分别列出。

| 配置 | 原始文本 token 数 | 运行时 prefill | 输出上限 / 实际 decode | 完成请求 |
|---|---:|---:|---:|---:|
| 容量 512，短输入 | 未单独采集 | 23 | 32 / 20 | 3/3 |
| 容量 512，22 条材料 | 384 | 397 | 64 / 20 | 3/3 |
| 容量 1024，短输入 | 未单独采集 | 23 | 32 / 20 | 3/3 |
| 容量 1024，54 条材料 | 896 | 909 | 64 / 20 | 3/3 |
| 容量 2048，短输入 | 未单独采集 | 23 | 32 / 20 | 3/3 |
| 容量 2048，118 条材料 | 1920 | 1933 | 64 / 20 | 3/3 |
| 容量 4096，短输入 | 未单独采集 | 23 | 32 / 20 | 3/3 |
| 容量 4096，246 条材料 | 3968 | 3981 | 64 / 20 | 3/3 |

八组均完成非空文本生成、无错误的流结束、会话与引擎关闭，进程退出 0。材料输入的 12 次回答均为 `The access code is AMBER-42. RAM stores temporary data needed by active programs.`，与材料内容相符。归档字段 `answer_contains_marker` 只检查大小写不敏感的子串包含关系；单个固定识别码和重复材料不构成通用检索质量评测。

下表保留每个引擎的首次请求；复用值为后两个新会话的中位数，括号为最小值至最大值。首个文本回调与运行时 decode 吞吐的定义沿用第二十一节。材料构造和分词在生成计时之前完成。

| 配置 | 首次首文本（ms） | 复用首文本（ms） | 首次 decode（tokens/s） | 复用 decode（tokens/s） |
|---|---:|---:|---:|---:|
| 512，短输入 | 4473 | 154.8（154.5–155.0） | 60.61 | 63.44（63.21–63.67） |
| 512，材料输入 | 4020 | 429.3（429.3–429.3） | 18.10 | 62.27（61.90–62.64） |
| 1024，短输入 | 3771 | 155.2（154.2–156.2） | 62.12 | 63.08（63.05–63.10） |
| 1024，材料输入 | 4137 | 784.0（781.3–786.7） | 18.49 | 59.23（59.19–59.26） |
| 2048，短输入 | 4468 | 173.6（172.5–174.7） | 52.19 | 53.55（53.05–54.05） |
| 2048，材料输入 | 4788 | 1611.7（1609.3–1614.0） | 50.11 | 53.51（53.42–53.60） |
| 4096，短输入 | 4932 | 184.8（181.1–188.6） | 52.41 | 54.09（53.74–54.44） |
| 4096，材料输入 | 7008 | 3193.2（3192.3–3194.2） | 52.46 | 53.73（53.67–53.78） |

主机侧引擎创建阶段为 24066–28785 ms，包含创建前记录开销，未计入首文本等待。材料输入的首次 decode 吞吐低于同引擎后续请求，现有记录没有分解其准备与执行开销。各配置只有一个进程，运行顺序固定。不能仅由表中差值计算上下文容量的独立影响，也不能把重复短响应外推为持续生成速度。

八个进程均未触发 critical 内存压力、18 GiB 进程 RSS 或 180 秒时限保护，未见 GPU 后端验证错误。压力采样合计 1020 次 normal、23 次 warning。其中，4096 短输入和材料输入分别有 5、6 次 warning。采样可能遗漏短时峰值，进程 RSS 也不是 GPU 分配量。本组验证范围到实际 3981-token prefill，尚未测量最大可用上下文、长时间运行或模型独占内存。

512／1024 的四组数据归档于 `experiments/data/2026-09-13/moe-context/`；2048 与 4096 各两组数据分别位于同日的 `moe-context-2048/`、`moe-context-4096/`。三个目录分别保存报告、完整流事件、原生日志、压力采样及材料原文与 tokenizer ID。`summary.json` 区分首次与复用请求，`manifest.json` 校验文件；两份采集脚本快照与各报告中记录的脚本哈希一致。复现见附录 C 第十二节。

## 二十三、完整 MoE 的较长输出（2026-09-13）

本组沿用第二十二节的 Apple M5 Pro、24 GiB 内存、macOS 26.5、v0.17.0 预编译包及完整 Gemma 4 26B-A4B GPU 量化文件。实际路径为 Artisan／Metal、F16，模型与动态库哈希未变。容量固定为 4096，benchmark 开启，thinking 与投机解码关闭，top-k=1、temperature=0、seed=42。输出上限 128、256 各使用一个新进程、新缓存目录，每个引擎运行三个新会话，串行执行。

两组使用相同英文提示词，要求用至少 600 词说明 RAM 与 SSD 的差别，涉及易失性、容量、访问延迟、带宽与虚拟内存。每次运行时 prefill 计数均为 58。六次 decode 计数分别恰为 128 或 256，均收到无错误的流结束事件，正常关闭会话和引擎，进程退出 0。

这六次是达到输出上限后的正常终止，正文都截在句中，尚未完成提示词要求的教程。冻结源码的停止检查同时处理停止 token、容量和输出限额，正常结束本身不能说明正文是否完整；定义见 `runtime/core/tasks.cc:93–111`。采集器的 `GENERATION_COMPLETED` 表示调用及清理完成，不是文章完整性或任务质量结论。

下表区分每个引擎的首次请求与后两个新会话。复用值为两次中位数，括号为最小值至最大值；总生成耗时从请求准备阶段到消费完结束事件，不含引擎创建。首文本与运行时吞吐沿用第二十一节的定义。

| 输出上限／引擎状态（请求数） | 首文本（ms） | decode（tokens/s） | 总生成耗时（ms） |
|---|---:|---:|---:|
| 128／首次（1） | 4150.0 | 38.72 | 6573.9 |
| 128／复用（2） | 172.1（170.1–174.0） | 52.25（52.24–52.26） | 2603.8（2601.1–2606.4） |
| 256／首次（1） | 3913.8 | 44.55 | 8827.6 |
| 256／复用（2） | 171.1（165.0–177.3） | 51.70（51.54–51.86） | 5104.8（5082.9–5126.7） |

两个进程的主机侧引擎创建阶段分别为 27089、24638 ms，包含创建前记录开销，口径同第二十一节。六次输出的相邻非空文本回调间隔最大为 22.7 ms，逐请求中位数、P95 与最大值保存在汇总中。间隔只覆盖首段至末段文本之间，包含回调调度和采集开销，不能解释为逐 token GPU kernel 时延。

压力采样为 316 次 normal、4 次 warning，未触发 critical、18 GiB 进程 RSS 或 180 秒时限保护，未见 GPU 后端验证错误。后台负载、文件页缓存和设备温度未隔离。单次生成只有数秒，本组不构成热稳态、长时间稳定性或能耗测量，进程 RSS 也不代表模型独占内存。

数据位于 `experiments/data/2026-09-13/moe-long-output/`，含完整响应、流事件、运行时指标、原生日志与压力采样。`summary.json` 保存首次与复用统计，`manifest.json` 校验文件，采集脚本快照与运行报告中记录的脚本哈希一致。复现见附录 C 第十三节。

## 二十四、完整 MoE 的同一会话多轮累积（2026-09-13）

本组使用第二十三节相同的设备、模型、运行时与生成配置，容量 4096，每轮输出上限 64。一个新进程创建一个引擎，依次建立三个独立会话，每个会话连续发送六轮请求。会话之间不传递历史，六轮内部始终使用同一会话对象；不重发拼接后的完整历史，也不清空会话。

每个会话使用不同的项目、城市与识别码。首轮建立三个字段；第 3 轮只更新识别码，第 5 轮只更新城市，其余轮次保持字段不变。每轮另附 20 条带编号的 RAM 背景说明，要求返回仅含 `project`、`city`、`code` 的 JSON。采集器解析完整响应并与独立保存的预期字典比较，不使用子串匹配。18 次结果全部相等，既保留了未更新字段，也使用了更新后的值。

逐轮运行时 prefill 为 377–397，decode 为 34–36，均未达到输出上限。会话计数接口在冻结版本中返回当前推理步位置，定义见 `runtime/conversation/conversation.cc:850–852`。本组每轮结束值都等于开始值加本轮 prefill 与 decode；下一轮开始值又等于上一轮结束值。下表的累计计数包含输入、格式与生成推进，不是对最终可见文本重新分词的结果。

三个会话均从 0 开始。第 2–6 轮的时延与吞吐取五次中位数，括号为最小值至最大值。ORION 首轮同时是引擎首次请求；LYRA、VEGA 首轮使用已经准备的引擎，不能视为另外两次独立首次启动。

| 会话 | 首轮后／第 6 轮后计数 | 首轮首文本（ms） | 第 2–6 轮首文本（ms） | 第 2–6 轮 decode（tokens/s） |
|---|---:|---:|---:|---:|
| ORION | 431／2516 | 4206.5 | 575.3（568.4–619.1） | 52.54（52.08–52.97） |
| LYRA | 432／2512 | 468.9 | 568.7（566.8–615.6） | 52.84（52.65–52.85） |
| VEGA | 431／2509 | 469.7 | 566.8（564.1–612.2） | 52.71（52.58–52.75） |

首文本等待逐轮从各自的请求准备阶段开始独立计时，包含回调构造与消息序列化，不含引擎创建阶段。引擎创建阶段约 26036 ms，包含创建前记录开销。运行时名为 TTFT 的字段始终使用该会话首次 prefill 和 decode，六轮内保持首轮值；不能将它当作后续轮次的首文本时延。表中吞吐使用本轮的 `last_decode_tokens_per_second`。原字段仍完整保存，定义沿用第二十一节。

18 次请求均收到无错误的流结束，三个会话和引擎正常关闭，进程退出 0。压力采样为 196 次 normal、4 次 warning，未触发既定保护，未见 GPU 后端验证错误。本组只验证三个固定状态更新任务及六轮累积，不代表一般对话质量、长时间运行或超出容量后的处理。

数据位于 `experiments/data/2026-09-13/moe-multiturn/`，逐轮保存提示词、预期字段、完整响应、会话前后计数、流事件与运行时指标。`summary.json` 按会话分组，`manifest.json` 校验原始文件和采集脚本快照。复现见附录 C 第十四节。

## 二十五、完整 MoE 的分阶段进程内存（2026-09-13）

本组使用第二十一节固定的完整 Gemma 4 26B-A4B GPU 文件、v0.17.0 预编译动态库及 Python 3.12.13。设备为 Apple M5 Pro、24 GiB 内存、macOS 26.5；实际路径为 Artisan／Metal、F16。容量固定为 4096，benchmark 开启，thinking 与投机解码关闭；top-k=1、temperature=0、seed=42，每轮输出上限 64。

三个新进程串行执行，各创建一个引擎、两个新会话。第一会话使用第二十一节的英文 RAM 短提示词，第二会话使用第二十二节的 3968-token 原始材料。每个进程的运行时 prefill 分别为 23、3981，decode 均为 20。六次请求均返回完整文本、收到无错误的 final，并正常关闭会话与引擎；三次材料回答均包含正确识别码。两个会话复用同一引擎，材料请求继承此前准备状态。

阶段采样使用 macOS `proc_pid_rusage` 的 `RUSAGE_INFO_V0`，直接记录 `ri_resident_size`、`ri_phys_footprint`、`ri_wired_size` 和 `ri_pageins`。前两项分别作为进程 RSS 与 physical footprint，单位换算为 GiB。它们是操作系统提供的不同进程统计口径，有重叠，不能相加。下表是三个进程在相同事件后的中位数，括号为最小值至最大值；“生成结束”包含 prefill 与 decode，没有单独捕获 prefill 完成时刻。

| 采样阶段 | RSS（GiB） | physical footprint（GiB） |
|---|---:|---:|
| 创建引擎前 | 0.042（0.042–0.042） | 0.019（0.019–0.019） |
| 引擎创建返回后 | 5.012（4.803–5.013） | 2.156（2.064–2.173） |
| 短输入生成结束后 | 0.200（0.101–0.327） | 2.977（2.975–2.978） |
| 材料输入生成结束后 | 0.229（0.130–0.354） | 3.184（3.182–3.185） |
| 引擎关闭返回后 | 0.262（0.259–0.264） | 1.700（1.685–1.708） |

引擎创建、两次生成和引擎关闭后，采集器还保存 `vmmap -summary` 的原始结果。阶段表先读取 libproc，再运行 vmmap，二者不是同一时刻的快照。引擎 API 返回后仍可能存在后端准备或释放活动，不能强行对齐两个工具的值。vmmap 摘要包含 `IOAccelerator (graphics)`、`owned unmapped (graphics)`、`mapped file` 等类别及换出列；这些工具类别没有与模型张量逐一对应。

材料输入完成后的 RSS 低于引擎创建后的 RSS，而 footprint 更高。这说明 RSS 下降不能直接解释为模型工作集减少，也不能据 footprint 约 3.184 GiB 得出模型只需这一容量。该数值未分解 GPU 分配、权重文件页面、KV cache 和临时区；关闭引擎后残留的进程统计也不足以证明内存泄漏，需要进一步追踪分配归属与释放时序。
