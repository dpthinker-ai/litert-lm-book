...
> 主矩阵采集于 2026-07-05，原有扩展实验采集至 2026-07-18；2026-09-05 新增的 HONOR 同机案例单列于第十四节。Mac 主基准记录共 30 次：18 次 backend × context 主矩阵，以及 12 次 `false`/`auto` 再采样；Android、自然文本 MTP、线程数和约束解码结果另列，不与主矩阵混算。原始记录位于 `experiments/data/`。
