...
- ✅ **KV cache 公式代入**（第 6 章）：结清，见下「六、模型实剖」——真实参数 24 层 / H_kv=2 / D=256（20 层）与 512（4 层）/ **int8**，每 token 28 KiB，4096 上下文约 117 MB。
...
- **KV 每 token = 2 × 2 × (20×256 + 4×512) × 1 B = 28,672 B = 28 KiB**；4096 上下文 ≈ 117 MB；静态槽位 32003 全预留 ≈ 918 MB。
