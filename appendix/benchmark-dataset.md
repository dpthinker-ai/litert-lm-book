...
主机侧引擎创建耗时为 24066–28785 ms，未计入首文本等待。材料输入的首次 decode 吞吐低于同引擎后续请求，现有记录没有分解其准备与执行开销。各配置只有一个进程，运行顺序固定。不能仅由表中差值计算上下文容量的独立影响，也不能把重复短响应外推为持续生成速度。
...
下表区分每个引擎的首次请求与后两个新会话。复用值为两次中位数，括号为最小值至最大值；总生成耗时从发起流调用到消费完结束事件，不含引擎创建。首文本与运行时吞吐沿用第二十一节的定义。
...
两个进程的主机侧引擎创建分别为 27089、24638 ms。六次输出的相邻非空文本回调间隔最大为 22.7 ms，逐请求中位数、P95 与最大值保存在汇总中。间隔只覆盖首段至末段文本之间，包含回调调度和采集开销，不能解释为逐 token GPU kernel 时延。
...
首文本等待逐轮从各自的流调用开始独立计时，不含约 26036 ms 的引擎创建。运行时名为 TTFT 的字段始终使用该会话首次 prefill 和 decode，六轮内保持首轮值；不能将它当作后续轮次的首文本时延。表中吞吐使用本轮的 `last_decode_tokens_per_second`。原字段仍完整保存，定义沿用第二十一节。
