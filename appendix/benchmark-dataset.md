...
本组沿用第二十六节的 Apple M5 Pro、24 GiB 内存、macOS 26.5、v0.17.0 预编译包及 Gemma 4 26B-A4B GPU 量化产物，完整模型与动态库哈希相同。实际后端为 Artisan／Metal，计算配置 F16；benchmark 开启，thinking 与投机解码关闭，top-k=1、temperature=0、seed=42。按下表顺序串行运行六个进程，每个进程创建一个引擎、三个新会话，并使用新的输出和缓存目录。操作系统文件页缓存未清除，其他应用负载未隔离。
...
六组均完成非空文本生成、无错误的流结束、会话与引擎关闭，进程退出 0。材料输入的九次回答均为 `The access code is AMBER-42. RAM stores temporary data needed by active programs.`，与材料内容相符。归档字段 `answer_contains_marker` 只检查大小写不敏感的子串包含关系；单个固定识别码和重复材料不构成通用检索质量评测。
...
主机侧引擎创建耗时为 24605–27469 ms，未计入首文本等待。材料输入的首次 decode 吞吐低于同引擎后续请求，现有记录没有分解其准备与执行开销。各配置只有一个进程，运行顺序固定。不能仅由表中差值计算上下文容量的独立影响，也不能把重复短响应外推为持续生成速度。

六个进程均未触发 critical 内存压力、18 GiB 进程 RSS 或 180 秒时限保护，未见 GPU 后端验证错误。512 短输入、1024 材料输入与两组 2048 输入各有 3 次 warning 压力采样，其余 744 次为 normal。采样可能遗漏短时峰值，进程 RSS 也不是 GPU 分配量。本组验证范围到实际 1933-token prefill，尚未测量最大可用上下文、长时间运行或模型独占内存。

512／1024 的四组数据归档于 `experiments/data/2026-09-13/moe-context/`；2048 的两组数据位于同日的 `moe-context-2048/`。两个目录分别保存报告、完整流事件、原生日志、压力采样及材料原文与 tokenizer ID。`summary.json` 区分首次与复用请求，`manifest.json` 校验文件；两份采集脚本快照与各报告中记录的脚本哈希一致。复现见附录 C 第十七节。
