...
5. 若该工作点已经落在 Roofline 的带宽约束区，算力翻倍不会改变带宽上限；提高带宽或减少每 token 访存量才可能提升上限。若 kernel、量化解包或同步仍受计算与固定开销影响，则算力变化仍可能改变实测吞吐。
...
2. 在两档 `B_eff` 相同、全部耗时都可折算为字节、256-token 档上下文成本忽略不计的假设下，`d_eq = 2.26 GB ÷ 4096 × (50.6 ÷ 45.6 − 1) ≈ 59 KiB/token`。它高于 28 KiB 的逻辑 KV 容量，因为这项等效估算还吸收了注意力计算、缓存行为、带宽利用率变化和其他随上下文增长的成本。它不是 DRAM 流量测量值。
...
4. `byte_offset = 3 × 4 B × 2048 = 24576 B = 24 KiB`。它只把本次写指针定位到当前输出张量的第 3 个 embedding 槽位之后。跨轮复用由 Session 上下文、模板后缀提取与 KV 状态承担；`byte_offset` 不保存历史，也不会跨调用自动累积。
...
1. 262144 × 2 B = 512 KiB；数据量约为 1.4 GiB 的 0.35‰。这个比例不能直接证明回传耗时可忽略，因为设备同步、映射和跨地址域传输与本地权重读取的成本模型不同。是否可忽略需要用分项计时验证。
...
6. 分组数减半为 28,125,000，每组 2 字节，scale 共 56,250,000 B。文件在填充前约为 `900,000,000 + 200,000,000 + 56,250,000 + 80 × 2^20 = 1,240,136,080 B`，约 1.15 GiB。加 224 MiB KV cache、0.40 GiB 运行开销和 0.25 GiB 余量后约为 2.02 GiB；若再保留一份同规模转换权重，约为 3.18 GiB。分组变大降低了 scale 开销，但“无完整副本时低于 3.0 GiB、有完整副本时超过 3.0 GiB”的结论没有变化。
...
10. 三个适配器物化后，输入 buffer 的逻辑 payload 为 `3 × 28.4375 = 85.3125 MiB`。切回 A 只改变 `current_lora_id_`，数值不减少。8 个适配器都物化后为 `8 × 28.4375 = 227.5 MiB`，尚未计入源数据视图、allocator 对齐和后端元数据。
11. 存储峰值为 `3.66 × 2 + 0.28 + 0.44 + 0.60 + 0.50 = 9.14 GB`，比 9.0 GB 配额多 0.14 GB。排空旧 Engine 后再创建新 Engine 不会删除旧模型、旧 cache、压缩包或余量，所以存储峰值不变；它避免新旧 Engine 工作集同时存在，运行内存峰值由“双 Engine 重叠”改为“两个 Engine 依次出现”。但 Engine 数量不是内存比例：页驻留、初始化临时量、会话和系统内存随时间变化，不能据此断言峰值减半。
...
7. 每步差值为 `128 ms ÷ 256 = 0.5 ms/step`。若按 1 MiB FP32 logits 折算，等效速率为 `1 MiB ÷ 0.5 ms ≈ 2.10 GB/s`。分母还包含两种 sampler 的计算、同步与等待，因此它不是互连带宽测量值；见第 8 章“如何观察同步与复制成本”。

## 第 9 章

1. 一轮归一化成本为 1.3，期望产出为 `1+p+p²+p³`。令二者相等，数值解约为 `p=0.233`；例如 `0.233+0.233²+0.233³≈0.300`。这只是 G=3、c=0.1 且条件概率恒定时的边界，不能外推为固定接受阈值。
...
3. verify signature 的 `input_pos` 维度是编译期固定的形状，G = 该维度 − 1（`runtime/executor/llm_litert_mtp_drafter.cc:256`）。改 G 意味着重新导出模型。
4. `r̂=ΣK/(RG)=0.4`，所以每轮平均接受 `G×r̂=1.2` 个草稿，连同 bonus token 平均返回 2.2 个 token。理论曲线中的 p 是在此前位置都匹配的条件下继续匹配的概率；聚合比例 r̂ 是接受前缀长度的样本平均，二者定义不同。
5. 前半是当前 token 的词嵌入（2560 维，来自 embedder 查表），后半是主干上一步的隐藏态 activation（2560 维，来自 decode/verify 的输出缓冲）。

## 第 10 章

1. `target_px = 256 × 16 × 16 = 65536`；`factor = √(65536 ÷ (768 × 512)) ≈ 0.408`。理想尺寸约为 313.5 × 209.0，按 `side_mult = 1 × 16 = 16` 向下对齐后得到 304 × 208；patch 数为 19 × 13 = 247。无输出 mask 且 `patch_num_shrink_factor = 4` 时，visual token 数为 `ceil(247 ÷ 4) = 62`。
2. 视觉、音频占位符分别为 −1、−2。执行管理器按模态 embedding 行数或有效 token 数插入等量占位符，`EmbeddingLookupMultiModal` 再逐行替换；见第 10 章图像、音频 embedding 替换两节。
3. 每个 token 的混合 KV 数据量为 `2 × 2 × (20 × 256 + 4 × 512) × 1 B = 28 KiB`。280 个 visual token 因而对应 `280 × 28 KiB = 7.65625 MiB`，约 7.66 MiB。`model_dimension = 2560` 是主干 embedding 宽度；KV 容量由各层的 KV 头数、头维度和数据类型决定，二者不能互换。
