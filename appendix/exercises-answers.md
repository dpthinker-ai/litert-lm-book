...
2. 在两档 `B_eff` 相同、全部耗时都可折算为字节、256-token 档上下文成本忽略不计的假设下，`d_eq = 2.26 GB ÷ 4096 × (50.6 ÷ 45.6 − 1) ≈ 59 KiB/token`。它高于 28 KiB 的 KV cache 理论大小，因为这项等效估算还吸收了注意力计算、缓存行为、带宽利用率变化和其他随上下文增长的成本。它不是 DRAM 流量测量值。
...
10. 三个适配器物化后，输入 buffer 的理论大小为 `3 × 28.4375 = 85.3125 MiB`。切回 A 只改变 `current_lora_id_`，数值不减少。8 个适配器都物化后为 `8 × 28.4375 = 227.5 MiB`，尚未计入源数据视图、allocator 对齐和后端元数据。
