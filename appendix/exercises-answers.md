...
4. 历史 v0.13.1 基准模型只有 128 与 1024 两档静态 prefill signature。当时的策略让 250、500、1000 token 都使用 1024 档，对应墙钟时间约 3.9 s；吞吐上升主要来自同一固定分块中的有效 token 比例提高。该记录未在 v0.17.0 重跑，不能套用当前 CPU/GPU 策略解释为新版曲线，也不能仅凭它判断算术强度或计算单元利用率。
