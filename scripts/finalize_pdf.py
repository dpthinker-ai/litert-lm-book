...
UNITS = [
    ...
    ("第 1 章 端侧 LLM 与 LiteRT-LM：三类物理约束与运行时版图", r"^\s*第\s*1\s*章",          "第一部分 · 约束、指标与系统概览", "第 1 章 三类物理约束"),
    ...
    ("第三部分 · 模型加载、异构执行与投机解码",  r"^\s*第\s*三\s*部分",       None,                         "第三部分 模型加载与异构"),
    ("第 6 章 KV cache：容量、带宽与会话生命周期", r"^\s*第\s*6\s*章",          "第三部分 · 模型加载、异构执行与投机解码", "第 6 章 KV cache"),
    ("第 7 章 模型文件与权重：量化、容器格式与 LoRA", r"^\s*第\s*7\s*章",     "第三部分 · 模型加载、异构执行与投机解码", "第 7 章 模型文件与权重"),
    ("第 8 章 异构算力：CPU、GPU 与 NPU",        r"^\s*第\s*8\s*章",          "第三部分 · 模型加载、异构执行与投机解码", "第 8 章 异构算力"),
    ("第 9 章 一次前向，多个 token：投机解码与 MTP", r"^\s*第\s*9\s*章",       "第三部分 · 模型加载、异构执行与投机解码", "第 9 章 投机解码"),
    ("第四部分 · 多模态、工具调用与跨语言集成",  r"^\s*第\s*四\s*部分",       None,                         "第四部分 多模态与集成"),
    ("第 10 章 多模态与工具调用：视觉/音频编码、约束解码与函数调用", r"^\s*第\s*10\s*章",         "第四部分 · 多模态、工具调用与跨语言集成",   "第 10 章 多模态与工具"),
    ("第 11 章 多语言绑定：C ABI、JNI 与 Embind", r"^\s*第\s*11\s*章",         "第四部分 · 多模态、工具调用与跨语言集成",   "第 11 章 多语言绑定"),
