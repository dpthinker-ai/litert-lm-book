...
> **版本注记**：v0.13.1 的 CLI 经 huggingface_hub 与 httpx 下载模型，经 SOCKS 代理联网时报缺少 `socksio`；v0.17.0 的下载模块只用标准库 `urllib`。本书在 v0.17.0 上复测四种代理环境，下载均成功，其中只设 `ALL_PROXY` 的一组是因 `urllib` 不使用该变量而直连成功；对照的 v0.13.1 在两种 SOCKS 环境复现了该报错。记录见附录 D 第二十六节。
...
从源码构建的读者可以用另一个入口：C++ 命令行入口程序 `litert_lm_main`（构建命令见第 12 章与附录 C）。用法和 Python 入口对应，指定模型文件与后端即可，后端默认是 `gpu`：
...
主矩阵中，上下文从 256 加长到 4096 后，cpu 的 decode 吞吐从 24.8 降到 20.7 tokens/s，gpu 从 50.6 降到 45.6 tokens/s。第 6 章按独立 KV 缓存组的张量形状算出，每个上下文 token 占 28 KiB。本节把已测得的减速折算成等效字节数，与这个容量值比较。它是基于简化模型的事后核对，不能单独识别减速原因，也不构成对另一长度或设备的性能预测。
...
107 和 59 分别是 28 的约 3.8 倍和 2.1 倍。这组结果不符合“两档有效带宽相同，新增成本仅为每步完整读取一份 KV”的简化模型。但 28 KiB 是容量口径，重复读取或布局转换可能使实际流量更大，不能据此排除 KV 访存这一原因。

\\(d_{\mathrm{eq}}\\) 将两档耗时差全部折算为字节。KV 重复读取、注意力计算增长和有效带宽变化都可能进入这个值，现有数据无法分离它们。它不是 DRAM 流量测量值；即使与 28 KiB 接近，也不能单独验证减速原因。第 6 章继续区分缓存容量、条件化扫描量与实际访问量。
...
| 20 | 图像怎样编码并进入语言模型？约束解码怎样限制结构化输出？ | 11 |
