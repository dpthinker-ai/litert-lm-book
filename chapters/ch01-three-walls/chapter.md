...
大模型生成文字分两个阶段，全书主线（第二部）会详细走这两步，这里先只借用它们的性能特征。prefill（预填充）把提示词一次性喂进模型，几百上千个 token 可以并行处理，算力越强越快，这一步决定"第一个 token 多久出来"。decode（解码）逐个 token 生成，每产生一个新 token，都要把模型的全部权重读一遍，算出下一个 token 的概率分布；它天然串行，第 N 个 token 没出来，第 N+1 个无从算起。
...
三类约束立起来了，尺子也有了。带着这两样往下走。想亲手验证那条 25 tokens/s 的上限、看它与实测差多少，第 2 章会在真机上跑一次，并给出用 Roofline 模型解读性能数字的分析框架。想知道约束背后模型如何一步步生成 token，第二部（第 3-5 章）会走完推理流水线（prefill → decode 的完整链路）。想看这三类约束具体如何被优化放宽，第三部（第 6-9 章）逐一展开。
