...
以离线邮件改写为例，模型和输入数据都在手机上。用户可能等待一段时间才看到首个 token，随后生成速度只有每秒数个 token。即使模型结构相同，服务器与手机的响应速度也可能相差很大，因为二者的内存、计算资源和功耗预算不同。
...
本章使用便于复算的示例参数，并逐处标明假设。模型文件大小不等于进程峰值内存，硬件标称带宽不等于有效带宽，单项能耗估算也不等于整机续航。附录 D 给出本书的实测数据，第 6 至第 9 章再结合 LiteRT-LM 的实现分析这些差异。
...
这个计算仍是上限估算。缓存命中、预留区是否实际读取、注意力 kernel 和 KV 数据类型都会改变 DRAM 流量。附录 D 给出了 Gemma 4 E4B 的对应基准。上下文从 256 增至 4096 时，CPU decode 从 24.8 降至 20.7 tokens/s，约下降 17%。该现象与 KV 读写量和注意力工作量随上下文增长的分析一致。仅凭端到端吞吐不能把降幅全部归因于 KV cache。
...
硬件峰值带宽由设备决定，软件仍可提高有效带宽利用率，或减少每步读取的字节量。prefill 常因权重复用而更接近算力约束区，batch=1 的稠密 decode 则通常主要受带宽约束；二者都需要用实际 Roofline 工作点确认。量化与推测解码能否提高吞吐，还取决于 kernel、接受率和额外计算，不能只凭方法名称判断。
...
20 pJ/byte 是示意假设，不是本书设备的实测值。0.04 J/token 也只包含题设中的 DRAM 搬运。它不含算术、量化解包、显示、操作系统、无线电和电源转换损耗，不能据此给出手机续航或可生成 token 总数。整机能耗需要在目标设备上测量。
...
配置评估必须同时检查内存预算、Roofline 工作点、持续功耗和后端支持。量化减少权重 payload，也可能增加解包计算或改变后端支持范围。推测解码试图用一次目标模型调用确认多个 token，但 drafter 与验证也有额外成本；`LiteRT-LM#2227` 记录了特定 GPU 条件下的负收益。[^ch01-issue-2227]
...
内存预算必须同时容纳权重 payload、KV cache、激活、后端工作区和运行时开销。低比特量化是减少权重 payload 的常用选择，但是否可运行仍要以目标后端的峰值内存为准。
...
第 2 章用 benchmark 与 Roofline 解读实测数据。第 3 至第 5 章说明从输入到输出的推理流水线。第 6 至第 9 章再分析 KV cache、模型加载与量化、异构后端和推测解码。
