...
prefill 为什么比 decode 快一个量级？根源在于一次前向同时处理整段提示词：读一遍权重，服务所有位置的计算，算术强度因此通常高于一次只处理一个 token 的 decode step（Roofline 分析见 1.4 节与 2.4 节）。不过“通常”不是“必然”：序列长度、模型结构和后端都可能使瓶颈变为内存访问或 host 侧开销，确认是否受算力约束，要靠目标设备的性能计数器或受控实验。
