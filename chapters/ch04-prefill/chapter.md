...
在代码里，这一步的编排入口是 `Prefill`（`runtime/core/tasks.cc:413 @ v0.13.1`）。它做的事概括起来很直白：校验输入不超长，把 token 交给执行器（executor），executor 调 LiteRT 的模型把这段 token 一次性算完，顺带把它们的注意力中间结果写进 KV cache——为 decode 铺好底。

## 一道选择题：固定长度还是可变长度

真正有意思的是 executor 怎么"吞"这段 token。这里有一道端侧特有的选择题。
...
这两个设计放在一起，才凑出"立刻停"的手感。它背后还站着一层异步底座：prefill 任务并不总在主线程同步跑，而是可以提交到一个执行队列、由工作线程处理（第 8 章会展开这层线程模型）。任务之间还连着依赖链，保证 decode 一定在它依赖的 prefill 完成之后才开始。异步 + 可取消 + 限长，三者合起来，是端侧"响应要跟手"这个体验诉求在架构上的落点。

## 小结

prefill 是算力受限的一步，快，且有静态/动态两条实现路径应对端侧固定形状的约束。更重要的是，它不是一个"跑起来就没法管"的黑盒——取消标志和限长开关让它随时可被打断，异步底座让它不阻塞主线程。这些设计都不是为了 benchmark，而是为了真实设备上"跟手"的体感。
