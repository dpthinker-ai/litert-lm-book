...
这两个设计放在一起，才凑出"立刻停"的手感——这是第 9 问答案的一半；另一半在 decode：生成循环每一步也都在看这个取消标志（第 5 章的 ShouldStop）。它背后还站着一层异步底座：prefill 任务并不总在主线程同步跑，而是可以提交到一个执行队列、由工作线程处理（第 8 章会展开这层线程模型）。任务之间还连着依赖链，保证 decode 一定在它依赖的 prefill 完成之后才开始。异步、可取消、限长三者合起来，端侧才有"响应跟手"的体感。
