...
真实模型的入口集比这个假想例子更粗。本书基准模型 Gemma 4 E4B 实剖出的 prefill signature 恰好只有两个：`prefill_1024` 与 `prefill_128`（附录 D 的段表）。对它，1300 个 token 的工单是 [1024, 1024]：剩余的 276 装不进 128，只能用 1024 收尾，第二段填充 748 个位置，整段填充率约 37%。入口越少产物越小、编译越省，但锯齿曲线的齿也越深，这正是上一段那个 TODO 想用实测成本去调优的空间。
