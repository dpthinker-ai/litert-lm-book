...
这一步是本章设计上的高潮：它把"逻辑上每轮都是全量历史"翻译成了"物理上每轮只处理增量"。表面看只是个字符串相减，背后接住的是整个 KV cache 复用的收益。
...
输入侧的两个关键设计，都值得记住：Engine/Session 的两级抽象（把昂贵的权重和廉价的对话状态分开），以及模板 diff 增量渲染（把逻辑全量翻译成物理增量）。它们一个管空间、一个管时间，共同为下一步做好了铺垫。
