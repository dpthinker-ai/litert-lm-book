...
- **Engine 重**。它持有模型权重——第 1 章那笔账的量级，以 GiB 计。加载一次要几秒、占几 GiB 内存。它应该被创建一次、长期复用。
...
还差最后半步说破：token id 只是编号，进模型前会先经查表变成一个高维向量——**embedding**（嵌入）。id 是名字，embedding 才是模型真正计算的对象。这半步平时不用你操心，但记住它：第 10 章讲图片怎么进模型时，它会成为主角。
