...
小预算减少图像空间采样点，可能丢失细节，这是模型质量问题，运行时不会把它报告为错误。大预算仍会受到 `max_num_patches` 限制，不能仅凭预算大于某个 signature 的容量就判断输入会失败。多 signature 选择路径按实际 patch 数折算选择长度。若该长度超过所有可用视觉入口的容量，函数才会报告容量不足，避免静默截断。10.2.5 节给出同一图像的两档预算实测；该案例只覆盖简单图形识别，不提供通用推荐数值。
