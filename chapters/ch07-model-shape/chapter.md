...
质量是另一项独立指标。INT8 或 INT4 的位宽本身不能限定质量损失，评估时必须给出模型、量化方法、校准数据与评测集。本书没有同一模型的 FP16、INT8、INT4 对照实验，因此既不报告质量收益，也不把质量损失分成“低”或“可接受”。同一 E4B 文件中的 INT2、INT4、INT8 张量属于一份混合表示，不构成三档模型对照。10.2.5 节的视觉预算对照没有改变权重，也不能用来评价量化损失。
