...
共享分支还要另外加上。其三组矩阵每层包含 \\(3\times2816\times2112=17{,}842{,}176\\) 个参数。将共享分支与选中的 8 个路由专家相加，单 token、单层的前馈矩阵计算约为 130.84 MFLOPs，30 层约为 3.93 GFLOPs。这个结果仍不含注意力、路由器、激活函数与加权合并；前面的 64 和 4 也只描述路由专家权重项，不是整个前馈层或完整模型的算术强度。参数量还可以核对一遍：30 层共享分支合计 \\(30\times17{,}842{,}176\approx0.54\\)B，加上路由专家后全部约 23.37B、选中部分约 1.96B，与模型卡的 25.2B、3.8B 相比差额都约为 1.83B。两个差额一致，对应注意力、embedding 与路由器这些每个 token 都参与的参数，说明本节的矩阵口径与模型卡口径自洽。
