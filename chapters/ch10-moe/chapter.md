...
输出上限与实际生成长度也须核对。在上述 M5 Pro、量化 GPU 产物和 Artisan／Metal F16 配置下，本书固定容量 4096，要求模型展开说明 RAM 与 SSD。128、256 输出上限各执行三个新会话，运行时 decode 计数均达到相应上限。复用引擎后，两组吞吐中位数为 52.25、51.70 tokens/s。六次流均正常结束，但正文均截在句中。因此，这组数据验证了较长的连续生成，不代表文章自然完成。首次请求、完整输出及计时范围见附录 D 第二十八节。

重复新建会话不能验证对话历史的累积。同一设备、产物与后端在容量 4096、每轮输出上限 64 下，另完成三个会话、每会话六轮的状态更新任务。各轮追加材料，第 3、5 轮分别更新识别码与城市，要求保留其他字段。18 次回答的 JSON 字段均符合预期；最终会话计数为 2509–2516。第 2–6 轮的首文本等待为 564–619 ms。这些轮次复用同一会话，计数包含已处理的输入与输出，不能当作单轮输入长度。逐轮数据及字段定义见附录 D 第二十九节。
