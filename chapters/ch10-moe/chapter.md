...
完整模型已有独立的生成验证。本书在 M5 Pro、24 GiB 内存、macOS 26.5 上，用 v0.17.0 预编译包运行上述 GPU 产物。实际路径为 Artisan／Metal，计算配置为 F16，thinking 与投机解码关闭。英文短句配置使用 128 上下文容量，三个进程各完成三个新会话；中文配置使用 256 容量，一个进程完成三个新会话。12 次请求均返回完整文本、收到无错误的流结束标记，并正常关闭会话与引擎。运行时的 decode 计数分别为每次 20、83，完整条件与输出见附录 D 第二十六节。这验证了该产物的两组短文本配置。它不是本地冻结源码构建验证，也不证明本节分析的自定义专家构图路径覆盖了该产物。

运行是否完成还取决于当时的资源条件。该产物此前出现过引擎和会话创建成功、生成期间却触发系统内存压力保护的情况；128 容量下也曾只有一次 `OK` 短响应成功。相关记录见附录 D 第二十、二十五节。减少同机应用负载后的重试取得上述 12 次成功，压力采样均为 normal。由于其他负载、文件页驻留与缓存状态未受控，不能把差异全部归因于关闭应用，或仅由上下文容量解释。API 创建完成与首次生成完成仍须分别记录。

模型文件名与浏览器运行时也要分别核对。模型卡所链接的演示，在本书冻结的版本中使用 `-gpu` 文件，并加载随演示分发的 JavaScript／WASM 运行时。它不是 v0.17.0 预编译动态库，也不能由当前 Web SDK 文档推定其行为。[^moe-web-demo] 本书的 Web 测试中，26B 产物在引擎创建阶段触发持续 warning 内存压力保护；较小 E4B GPU 产物则完成生成。原生路径的成功不能替代 Web 路径验证，文件名带 `-web` 的另一份产物也未做完整运行测试。冻结资源和日志见附录 D 第二十四节。
...
首次请求与复用引擎后的请求应分开统计。在 10.6.3 节同一 M5 Pro 和量化 GPU 产物上，128 容量、输出上限 32 的英文短句测试使用 Artisan／Metal、F16，并开启 benchmark。三个进程的首次请求，首个非空文本回调在调用后 1650–2813 ms 到达；同引擎后续六个新会话为 120–125 ms。后者的运行时 decode 吞吐中位数为 82.6 tokens/s，每轮 decode 计数为 20。这些新会话使用相同提示词，复用了引擎已准备的状态。不能把后续会话的等待时间当作首次启动体验，也不能外推长时间生成的吞吐。计时定义和逐轮范围见附录 D 第二十六节。

上下文容量与实际输入长度也要分别记录。同一 M5 Pro、24 GiB 设备与量化 GPU 产物，沿用 Artisan／Metal、F16 和 benchmark 配置，在 512、1024、2048、4096 容量下均完成了短输入与较长输入测试。短提示词的运行时 prefill 计数仍为 23；增加材料后分别为 397、909、1933、3981，每组均完成三个新会话，每次运行时 decode 计数为 20。较长输入的 12 次回答都找回了材料开头的识别码。该任务验证了这些输入下的生成与简单信息提取，不是模型最大上下文或通用检索质量测试。首次请求、引擎复用、输出上限与压力记录见附录 D 第二十七节。
