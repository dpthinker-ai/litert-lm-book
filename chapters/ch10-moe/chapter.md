...
上下文容量与实际输入长度也要分别记录。同一 M5 Pro、24 GiB 设备与量化 GPU 产物，沿用 Artisan／Metal、F16 和 benchmark 配置，在 512、1024、2048 容量下均完成了短输入与较长输入测试。短提示词的运行时 prefill 计数仍为 23；增加材料后分别为 397、909、1933，每组均完成三个新会话，每次运行时 decode 计数为 20。较长输入的九次回答都找回了材料开头的识别码。该任务验证了这些输入下的生成与简单信息提取，不是模型最大上下文或通用检索质量测试。首次请求、引擎复用、输出上限与压力记录见附录 D 第二十七节。
