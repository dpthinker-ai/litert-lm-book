...
同一 26B GPU 产物在较小上下文配置下取得了完整生成记录。仍使用 M5 Pro 和 v0.17.0 预编译包，将上下文容量设为 128、输出上限设为 1，返回了 `OK`，引擎正常关闭。本次走 Artisan／Metal，未触发原生采集器的停止条件。它验证了该配置的一次短生成，不能替代首 token 时延、持续吞吐或质量评估。同机负载和缓存状态没有受控，因此不能将成功单独归因于上下文容量的变化。同样容量下，换用解释 RAM 的提示词、将输出上限设为 32 后，另一轮又在生成期间触发 critical 压力保护。两轮同时改变提示词和输出上限，没有隔离各自影响，完整模型的稳定生成仍待验证。补测记录见附录 D 第二十五节。
