...
本章引用的吞吐与内存实验沿用附录 D 的历史版本记录，尚未在 v0.17.0 重跑。模型文件的张量形状也沿用原产物，不能视为新版所有模型的配置。
...
因此，表 6-1 是所有层按完整 S 扫描的条件化估算，不是新版局部注意力路径的流量预测。是否真正减少分配和搬运，还要检查导出张量、元数据及目标后端；仅有局部 mask 不代表缓冲已缩小。
...
KV cache 以容量和带宽换取较少的重复计算。历史基准模型产物按统一序列宽度计算为 28 KiB/token；局部缓存和混合注意力模型则应逐层求和。固定形状路径中，`--max-num-tokens` 经 magic number 机制成为张量宽度。本章旧版同 prompt 实验的预留宽度从 4096 增至 8192，decode 从 26.4 降至 21.5 tokens/s，降幅约 19%；这组数据尚未在 v0.17.0 重跑。
