...
两点值得记住。其一，这不是"调大才变慢"，而是"默认就偏大"：256 token 的 prompt 按上面的推导默认预留 4096，decode 每步为近 16 倍于实际长度的 KV 宽度付访存。其二，过小不是变慢而是直接失败：工单恰好打满预留宽度时，越界在 prefill 阶段就以编译模型错误的形式抛出，不是静默截断。这个边界现象提示，预留宽度与工单形状的交互有暗坑，手动调参时值得留意。
