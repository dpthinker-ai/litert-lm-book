...
前面几章的推理流水线只处理文本：输入是 token 序列，输出逐 token 变成文本。本章在这条流水线的两端各增加一个环节。输入端，图像和音频要先经过模态编码与 embedding 查找，变成主干模型能接收的向量序列，再进入 prefill（11.1 至 11.3 节）；输出端，工具调用要求模型生成可解析的结构化文本，约束解码在 decode 采样时用文法状态屏蔽不合法的 token（11.4 节）。11.5 节按数据边界定位多模态输入的失败，11.6 节说明工具调用从解析到执行的信任边界。这两段对应第 2 章表 2-2 的问题 20。

本章源码分析采用 v0.17.0；来自附录 D 的模型检查与性能实验保留原始记录。真机和 Python SDK 实测使用 v0.13.1，未在新版本重跑，不能作为 v0.17.0 的性能结果。

## 11.1　从 Message 到 InputData：入口类型与所有权
...
`InputImage` 和 `InputAudio` 都禁止复制、允许移动，二进制数据或 `TensorBuffer` 的转移因此在类型上可见。两者的 `CreateCopy` 仍可显式复制：字符串做字节复制，`TensorBuffer` 调用 `Duplicate`，后者只复制句柄，底层缓冲是浅拷贝。调用方不能把 `Duplicate` 理解为重新分配并复制全部张量内容。

`InputImage` 的 `std::string` 分支持有自己的字节，`absl::string_view` 分支只保存视图，后者的源缓冲必须至少存活到预处理完成。这一点来自成员类型本身，没有额外的生命周期管理。Gemma 4 的 `path/blob` 路径在调用预处理器前构造 `std::string`，因此不会把文件映射的原始视图继续传入异步 prefill。

第三次转换发生在提交 prefill 时。预处理完成后，图像通常变为单个 `TensorBuffer` 或名为 `images`、`positions_xy` 的张量表，音频变为频谱 `TensorBuffer`。Conversation 把这组 `InputData` 交给异步 prefill 接口：接口接收 const 引用，在返回前生成一份内部副本（张量通过 `CreateCopy` 调用 `Duplicate`），再把这份副本移动到执行任务里。调用方不必让原始 `InputData` 存活到异步任务结束，但底层张量的复制仍是句柄级浅拷贝。
...
> 表 11-1　多模态输入在各阶段改变表示；排查悬空视图和 shape 错误时，应先确定对象处于哪一层。

## 11.2　图像输入的编码与 embedding 替换
...
执行管理器从视觉 embedding 张量的倒数第二维读取 visual token 数，再向组合 token 序列插入同样数量的占位符。prefill 查表时，非负 token 由文本 embedding 查找器处理，多模态查找器则依次把视觉 embedding 的行复制到占位符对应的位置，所以占位符数量与 embedding 行数必须一致。多张图的 embedding 会先合并，相应的占位符也按各图输出行数插入序列。

<figure>
{{#include figs/fig-11-1.svg}}
<figcaption>图 11-1　在带视觉适配器的路径中，图像经 patchify、视觉编码器与适配器得到 embedding；prefill 查表再按 kSpecialToken 位置写入对应行。</figcaption>
</figure>
...
### 11.2.2　visual token budget 如何作用于 patchify
...
预算过小与过大导致的问题不同。小预算减少图像的空间采样点，可能丢失细节，这是模型质量问题，运行时不会把它报告为错误；大预算会扩大 patch 上限，可能使输入超出已选视觉入口的容量。多 signature 路径按实际 patch 数折算所需长度；没有足够长的候选入口时，函数返回错误，避免静默截断。11.2.5 节给出同一图像的两档预算实测，该案例只覆盖简单图形识别，不提供通用推荐数值。

### 11.2.3　多图组合不是 batch

一条消息可以包含多幅图。每幅图分别执行预处理与视觉编码，得到各自的 `ExecutorVisionData`，执行管理器随后沿 token 维把它们拼接起来。假设两幅图分别产生形状 `[1,T_1,D]` 和 `[1,T_2,D]`，组合结果不是 `[2,T,D]`，而是 `[1,1,T_1+T_2,D]`；若输入已经是四维张量，则保留前两维，只累加倒数第二维。对应的占位符也按消息顺序插入同一文本序列。

这一区别影响三个判断。第一，多图不会自动变成可并行的图像 batch，当前组合函数先逐图编码，再拼接结果。第二，主干接收的是一条更长的上下文，而不是多条独立样本。第三，每幅图的边界要由模型约定的开始/结束 token 表达，不能从拼接后的 embedding 张量恢复。

组合一个对象时，函数直接移动原有对象；组合多个对象时，它创建新的 host `TensorBuffer`，依次锁定每个源缓冲并复制其 packed bytes。因此，单图和多图经过的内存路径不同。源码只能证明多图存在一次组合复制，不能据此估计它在端到端时延中的占比。
...
这与 6.1 节的 28 KiB/token 一致：256 个 token 正好 7 MiB。三次同样的输入会增加 768 个序列位置，对应 21 MiB 的活动 KV 数据。这里计算的是有效上下文对应的数据量或容量需求；固定宽度实现已经按预留宽度分配了 KV buffer（6.2.2 节），插入图像未必使进程驻留内存再增长 7 MiB。prefill 时延还受注意力、后端与 padding 影响，需要真机测量。图像预算应同时考虑 `max_num_patches` 与 `patch_num_shrink_factor`，前者不能直接等同于 visual token 数。

### 11.2.5　真机案例：同一图像的两档预算

一张图片占多少上下文，不能只看原始分辨率。附录 D 第十五节用同一张 640 × 480 RGB 图像，在 HONOR MEP-AN00 上运行 Gemma 4 E4B。图中从左到右是红色正方形、蓝色圆形和绿色三角形，提示词要求按顺序描述。主干与视觉编码器使用 GPU OpenCL，视觉适配器使用 CPU/XNNPACK；上下文上限为 4096 token，MTP 关闭。

预算设为 70 时，预处理日志记录缩放到 432 × 336，得到 567 个 patch。预算为 280 时，图像反而放大到 912 × 672，得到 2394 个 patch。这与 11.2 节的尺寸公式一致：它按预算和长宽比计算目标尺寸，没有把目标尺寸限制在原图以内。放大增加了计算位置，并不会增加原图没有的细节。
...
首次请求还有另一类开销：引擎创建耗时约 24274 ms，之后首次请求中的文件读取、会话配置与创建合计约 7331 ms，后续请求的这一项只有约 3–4 ms。日志在首次 Conversation 创建期间记录了视觉执行器初始化，只计 Engine 创建会遗漏这部分准备时间。从请求开始计，首次请求的首段文本等待约 8662 ms，不能与后续请求混算成固定的“图片时延”。计时范围、逐请求内存及错误案例见附录 D 第十五节。

## 11.3　音频输入的频谱编码与 embedding 替换

音频 embedding 采用另一种占位符：`ExecutorAudioData::kSpecialToken` 为 -2，embedding 行数同样应与音频占位符数量一致。执行管理器根据音频执行器报告的有效 token 数插入相同数量的 -2，prefill 查表再由多模态查找器复制音频 embedding。文本、视觉和音频由不同查找器提供向量，但主干接收的都是同一种 embedding 序列。

### 11.3.1　DSP（数字信号处理）前端与分块编码
...
附录 D 的模型文件检查记录显示，音频编码器与 `audio_adapter` 分属两个模型段；适配器输入 `features` 的形状为 `[1, 204, 1536]`。其中 1536 是每个位置的特征维度，204 是编码器输出的特征序列位置数。204 不是原始波形帧数，也不能仅凭该张量形状还原 log-mel 输入帧数，二者之间还有编码器的缩减过程。该记录只能支持“编码器输出再进入适配器”；适配器输出形状仍须检查输出张量，不能由 1536 维的输入直接推出。

### 11.3.2　音频状态、分块边界与有效 token

音频预处理器可能保存跨调用状态。它的重置操作会清空输入队列，再按是否采用半因果 padding 重设下一帧所需样本数。Gemma 4 数据处理器每处理完一个音频对象便调用重置，然后插入 `InputAudioEnd`，因此同一消息中的两个完整音频文件不会在 DSP 输入队列中直接相接。
...
> 表 11-2　多模态 shape 只能支撑相邻阶段的结论；跨过编码器、池化或适配器反推时需要额外配置。

## 11.4　约束解码：按文法屏蔽 token
...
代码行 `(1)` 屏蔽当前状态不允许的 token，`(2)` 采样器从剩余候选中选出下一个 token，`(3)` 用该 token 推进状态。构造函数为 batch 中的每条序列分别启动一个约束状态，因此各序列维护独立状态。

### 11.4.1　状态推进发生在下一次采样之前
...
这里 \\(z_t\\) 是模型 logits，\\(A_t\\) 是允许 token 集合。公式中的三个动作分属约束引擎、采样器和状态推进，顺序不能交换：若先采样再屏蔽，非法 token 已经选出；若省略推进，每一步都会重复使用起始状态。
...
### 11.4.2　工具声明生成的文法覆盖到哪里

解析器文法与采样文法用途不同。ANTLR 的 FC parser 文法在模型生成完之后使用，它把任意合法 `ID` 解析为函数名或参数名；约束生成器则读取本次 Conversation 的工具声明，构造更窄的 llguidance 文法。二者不能混为同一层保证。

FC 约束生成器逐个读取工具的 `name`，只为声明过的函数名生成 `"call:" "tool_name" ...` 分支。对参数，它读取 `parameters.required` 和 `parameters.properties`，为必需参数与可选参数分别建立规则。因此，在这条由工具声明派生的路径中，函数名和已声明参数名不只是任意 `ID`。
...
> 表 11-3　约束把部分工具声明编译进 token 允许集合，但不会完成授权、业务校验或外部执行。
...
<figure>
{{#include figs/fig-11-2.svg}}
<figcaption>图 11-2　约束解码按当前文法状态屏蔽 token；图中只表示 FC 结构约束，不表示函数存在、参数语义正确或调用可执行。</figcaption>
</figure>

## 11.5　按数据边界定位多模态输入失败

多模态请求报错时，最后出现的 `prefill failed` 往往只说明任务没有完成，不能定位最初的错误。有效的排查顺序应沿数据变换方向进行：消息对象、模板标记、预处理张量、模态执行器、embedding 组合、主干 prefill。每经过一层，数据表示和责任方都会变化。
...
这个错误首先要检查两项信息：原始消息中图像项的数量与顺序，以及本轮渲染 prompt 中模态标记的数量与顺序。增加 GPU 日志、替换后端或调整 decode 参数都不会改变结果。相反，若队列处理完后仍有图像，函数会返回 `Provided more images than expected in the prompt.`。两条错误分别对应“标记多于对象”和“对象多于标记”。
...
音频可以用同一方法分层。消息与模板数量错误发生在数据处理器；音频格式解码、PCM 分帧和 log-mel 计算发生在预处理器；频谱宽度与 mask 长度检查发生在音频执行器；最后才是 audio embedding 与 -2 占位符组合。执行管理器若收到未经预处理的 `InputAudio`，会返回 `The audio is not a preprocessed tensor.`。
...
<figure>
{{#include figs/fig-11-3.svg}}
<figcaption>图 11-3　多模态输入依次经过六个数据边界；每层应记录输入表示、输出 shape 与第一条错误。</figcaption>
</figure>

## 11.6　Tool Use：结构解析与应用执行

Tool Use（工具调用或函数调用）把模型生成的结构化文本转换为函数名和参数。函数由应用实现并执行，LiteRT-LM 负责格式化工具声明、检测输出并解析调用。[^ch10-tooluse-doc]

一次工具调用包含以下环节：

1. 声明工具。第 3 章 `Preface` 的 `tools` 字段携带工具声明；`Preface` 的消息、工具和附加上下文都用保留插入顺序的 JSON 对象表示，格式化时不会按键名重新排序。
...
4. 解析与执行。FC 路径的 lexer 与 parser 由 ANTLR 从文法生成为 Rust 代码，C++ 通过 CXX bridge 接收结果。文法只有六条规则：
...
代码行 `(1)` 要求 `call`、冒号、`ID`，参数对象可选；`(2)` 的 `value` 递归引用 `object` 和 `array`，因此能表示嵌套参数。解析函数把上面的示例还原为 `{"name":"tool_name","arguments":{"param_1":7,"param_2":"foo"}}`，头文件给出了相同的输入输出。FC 与 Python 风格调用使用 ANTLR parser，JSON 路径调用 `serde_json::from_str`。

解析成功后，应用核对函数与参数，执行函数，再把结果作为消息发回模型。执行函数是应用的责任，不是语法约束或 parser 的责任。

| 环节 | 职责 | 承担者 |
|---|---|---|
| 1. 声明工具 | `Preface.tools` 携带可用工具描述 | 应用配置，Conversation 保存 |
| 2. 格式化 prompt | JSON 值转换为目标模型使用的 FC 格式 | 模型数据处理器 |
| 3. 约束生成 | 按当前约束状态屏蔽 token | 约束解码器与 llguidance |
| 4. 解析与执行 | parser 返回函数名和参数；应用校验、执行并回填结果 | FC parser 解析，应用执行 |

> 表 11-4　Tool Use 各环节的职责边界；约束生成、结构解析与应用执行是三个不同阶段。

### 11.6.1　parser 输出仍位于信任边界之外

模型输出不是函数调用对象。FunctionGemma 的数据处理器先取得完整响应文本，只有 `Preface` 中存在工具时才解析工具调用；解析成功后，它分别把普通文本与 `tool_calls` 写入 assistant message。

解析分两步：先用模型配置的起止 fence 切分文本与工具代码块，再根据语法类型选择 Python、JSON 或 FC parser。默认策略在工具块解析失败时返回错误；若关闭 `return_error_on_parse_failure`，函数把原代码块作为文本返回，并附加 `error` 字段。因此，应用必须区分三种结果：没有工具块、工具块解析失败、解析成功并得到 `tool_calls`。

解析成功只表示字符串满足 parser 接受的语法。parser 不访问用户身份、设备状态、网络权限或外部服务。FC 解析器把 `ID` 保存为 `name`，把可选对象保存为 `arguments`，没有参数对象时填入空对象。这些字段仍来自模型输出，不能直接作为函数指针、文件路径或 SQL 片段使用。
...
### 11.6.2　端到端案例：受控地修改设备模式

下面的 `set_device_mode` 是应用侧集成示例，不是 LiteRT-LM 内置工具。它修改外部设备状态，因而适合展示结构约束与执行授权之间的差别。工具声明可以写成：
...
FunctionGemma 的格式化代码会把工具声明转换为模型使用的 FC 格式。启用约束后，约束生成器把工具 JSON 与 fence、引号等格式选项传递给 Gemma 的约束提供者。模型可能生成：
...
由工具声明派生的文法可以把函数名限制为 `set_device_mode`，并把 `mode` 限制在声明的枚举中。parser 随后产生：
...
> 表 11-5　parser 只跨过语法边界；真正的信任边界位于宿主的命令校验与授权入口。
...
<figcaption>图 11-4　模型文本和 parser JSON 均属于不可信输入；宿主校验、授权与受控适配器共同构成执行边界。</figcaption>
</figure>
...
图像和音频从消息进入模型数据处理器，依次变为预处理张量、模态 embedding 和主干输入。执行管理器插入模态占位符，prefill 查找器再写入对应向量。排错时应沿相同顺序检查对象数量、shape、signature 与 embedding 行数。
...
约束解码按状态计算允许 token 位图，并在采样前屏蔽其他候选。由工具声明派生的文法可以编码函数名、顶层参数和部分类型，但不覆盖完整 schema。Tool Use 还需要 parser 把文本还原为函数名和参数；parser 结果仍是不可信输入，应用随后完成白名单、完整 schema、权限、幂等与实际执行。
