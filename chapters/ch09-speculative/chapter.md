...
> 使命：讲透"用猜的反而更快"这件反直觉的事——drafter 怎么草拟、base 模型怎么一次验证一串、以及快多少为什么全看接受率。这是第三部的压轴，也是端侧凿带宽墙最漂亮的一招。

第 1 章立带宽墙时，给过两条出路：提高带宽（端侧作者说了不算），或减少每 token 要读的字节（量化，第 7 章做了）。这一章是第三条，也是最巧的一条——**别改带宽，也别改模型，改成一次前向多吐几个字**。这就是第 2 章"二十个问题"的第 19 问：推测解码靠"猜"，为什么反而更快？什么时候又更慢？

## 一次昂贵前向，能不能多榨几个字

先把账摆清。decode 的瓶颈是带宽：每生成一个 token，都要把全部权重读一遍（第 1 章）。这一次前向很贵，而它只换来一个字。问题自然浮现：**这一次昂贵的前向，能不能一次多榨出几个字？**

推测解码的回答是：能，只要你愿意先"猜"。找一个又小又快的模型先草拟接下来的几个字，再让大模型**一次前向**把这几个字一起验一遍。猜对的部分白赚，猜错的地方用大模型的正确答案兜底。关键在于——验证那几个草拟字，只花大模型**一次**前向，而不是几次。一次昂贵前向，摊到了多个字上。
...
- 接受率高时：一次 base 前向出好几个字，而 base 前向是最贵的那一项——于是每个字摊到的成本大降，明显更快。官方报告 Gemma 4 上可达约 3 倍（官方博客口径，属【文档】级；本书的实测核对需要一个支持 MTP 的 Gemma 4 类模型，基准数据集采集后回填〔基准 D〕）。
- 接受率低时：草拟大多被丢弃，drafter 那几步白做，还多搭了 verify 的开销。如果这些额外开销超过了省下的前向，净结果就是**更慢**。

这正是那个反直觉现象的出处（第 19 问后半，`LiteRT-LM#2227`）：在某些 GPU（如 PowerVR）上，MTP 反而拖慢了 decode。成因可以从上面的账推出来——当接受率不够高、或 drafter/verify 在那块硬件上的相对开销偏大时，这笔赌注就亏了。（该现象无真机可复现，按上游报告的【文档】级引用；成因是基于本节经济账的推断。）
...
推测解码是凿带宽墙的第三招，也是最巧的一招：不改带宽、不改模型，靠一个便宜的 drafter 先猜、base 模型一次前向验一串，把最贵的那次前向摊到多个 token 上。bonus 设计保证它论 token 数不亏，但接受率决定它到底快多少——赌 drafter 猜得准，赌赢了成倍加速，赌输了（接受率低、开销大）反而更慢。它是一个有条件的加速，而不是免费的午餐。
...
<!-- 实测（MTP 开/关、文体接受率）待基准 D，需支持 MTP 的 Gemma 4 类模型（非基准用的 3n E2B）。#2227 无真机，成因为经济账推断、按【文档】级引 issue。图 9-2(接受率-收益曲线) 表 9-1(开/关实测) 需实测数据，待基准 D；本轮出签名图 9-1(时序)。 -->
