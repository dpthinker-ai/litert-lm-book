...
推测解码有几种形态：草稿模型可以是完全独立的另一个模型，也可以是和主模型共享主干的多头结构（MTP，Multi-Token Prediction，Gemma 4 走的就是这一路）。在 LiteRT-LM 的运行时里，drafter 装载为一个独立的小模型，验证则用 base 模型上一个专门的 `"verify"` signature（`runtime/executor/llm_litert_mtp_drafter.cc:63`、`:228 @ v0.13.1`）——所以不管训练时共不共享主干，运行时看到的都是同一套"小模型草拟 + 大模型验证"。
