...
第 1 章给出缓解 decode 内存带宽约束的两条思路：提高有效带宽，或减少每 token 读取的字节数；量化属于第二条。投机解码（speculative decoding）尝试让基础模型一次前向确认多个 token，以分摊权重读取成本。实际收益还取决于草拟、验证与调度的耗时。本章据此定量分析第 2 章表 2-2 的问题 19：投机解码为什么可能加速，又会在什么条件下减速。前半章（9.2 至 9.7 节）按代码依次说明草拟、验证、接受与集成，后半章（9.8 至 9.10 节）计算加速比、处理多 token 返回带来的停止检测问题，并说明开启条件。

## 9.1　投机解码的目标：一次验证多个候选 token

稠密模型在普通 decode 阶段一次前向通常只确定一个 token，却需要访问模型权重。投机解码要改变这个比例：让基础模型一次前向确认尽可能多的 token。

做法分两步：先由低成本的 drafter 草拟若干 token，再由基础模型（base 模型）用一次前向验证全部候选。比对之后，最长的匹配前缀全部接受，第一个不匹配的位置改用基础模型自己给出的 token。在权重流量近似一次完整扫描的简化模型中，一轮接受多个 token 就能分摊这笔读取成本。源码能确认一次验证调用；实际权重流量还取决于后端的分块与缓存行为，需要设备测量。
...
> **版本注记**：v0.17.0 在 M5 Pro、Gemma 4 E4B 上补测了显式开关，prefill 1024、decode 128、KV 容量 8192，各条件预热后测 3 次。CPU 8 线程的 decode 中位数由关闭时的 26.8 降至开启时的 11.2 tokens/s；GPU WebGPU/Metal 由 67.4 降至 47.8 tokens/s〔基准 D，第十六节〕。输入为短字符串分词后补足 token 的合成负载，开启组日志中的聚合接受比例均为 0。这组结果不能预测自然代码或对话的收益，也不能分离各执行阶段的成本。

若 base 模型采用 MoE，一轮验证的候选 token 可能访问不同专家。拒绝候选不会撤销已经支付的专家计算与读取成本；验证耗时需要按专家并集重新分析，见 10.4 节。
