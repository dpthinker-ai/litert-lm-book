...
- 接受率高时：一次 base 前向出好几个字，而 base 前向是最贵的那一项——于是每个字摊到的成本大降，明显更快。官方报告 Gemma 4 上可达约 3 倍（官方博客口径，【文档】级）。本书的主基准就是支持 MTP 的 Gemma 4 E4B，用它开/关 MTP 对比来核对这个数，待采集后回填〔基准 D〕。
...
<!-- 实测（MTP 开/关、文体接受率）待基准 D，主基准 Gemma 4 E4B 本身支持 MTP，直接开/关对比。#2227 无真机，成因为经济账推断、按【文档】级引 issue。图 9-2(接受率-收益曲线) 表 9-1(开/关实测) 需实测数据，待基准 D；本轮出签名图 9-1(时序)。 -->
