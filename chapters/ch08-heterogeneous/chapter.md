...
本章引用的实测数据保留附录 C、D 的原始版本记录。v0.17.0 的 Mac CPU/GPU 主矩阵另列于附录 D 第十六节；线程扫描、手机持续运行与 NPU 部署实验尚未重跑。
...
上游 issue `LiteRT-LM#2281` 报告：同一个 `.litertlm` 文件分别用 `--backend=cpu` 和 `--backend=gpu` 运行，速度不同，输出文本也可能不同。[^ch08-issue-2281] 这就是表 2-2 问题 17 的来源。

它说明后端不是一个只影响速度的开关。换后端改变的是整条计算路径：算子 kernel、数值精度处理和反量化实现都可能随之变化。LiteRT-LM 因此在运行时按配置选择后端，并把这些差异封装在同一个执行器接口之内；后面各节按这条路径的顺序展开。
...
CPU 通常有最宽的算子覆盖范围，也是没有可用加速器时的后备执行单元。LiteRT-LM 为 CPU 路径提供线程数配置，并在部分 Pixel Tensor 设备上设置线程亲和性。这两项配置影响调度方式，但不能脱离具体设备推导出统一的性能最优值。

### 8.3.1　CPU 亲和性：Pixel Tensor 的硬编码核表

移动 SoC 常把不同性能特征的 CPU 核组合在一起。LiteRT-LM 提供的亲和性工具执行三个步骤：识别当前是不是 Pixel Tensor 设备，查询预设的性能核编号，把当前线程绑定到这些核上。核编号不是运行时推导的，而是按芯片型号硬编码：
...
代码行 `(1)` 起每一行给出一款 Pixel SoC 的预设核编号：G3 使用 4 至 8 号核，G4 使用 4 至 7 号核，G5、G6 使用 2 至 7 号核。当前实现不分析运行时拓扑，只查这张版本内置表，表外设备不会自动得到同类配置。
...
这张表与属性检查把适用范围限定为已列出的 Google Tensor 芯片。对未知 SoC，查询性能核的函数返回空向量，绑核函数遇到空向量时记录 warning 并返回成功；非 Android 编译分支里，设备识别恒为 false，绑核函数也直接返回成功。因此，这段代码不能为 Qualcomm、MediaTek 或未列入表中的 Pixel SoC 自动选择 CPU 核。

绑核最终调用 Linux 的 `sched_setaffinity`：
...
GPU 适合并行执行矩阵运算。第 5 章介绍过设备侧采样（device-side sampling）：采样器直接消费 GPU 产生的 logits，不必在每个 decode step 把完整 logits 张量传给 host 侧的采样器。

先计算这部分传输量。decode 每一步都会产生长度等于词表大小的 logits，以本书基准模型的 262144 词表和 FP32 计算，张量大小为 262144 × 4 字节 = 1 MiB。host 采样要让这 1 MiB 对 CPU 可见，生成 512 个 token 就要处理 512 份；设备侧采样则在 GPU 侧消费它。它减少的只是完整 logits 的跨设备传输，选中的少量 token id 仍会复制到 host，用于返回结果和更新运行时状态。本书没有隔离测量该优化的吞吐或时延收益，不能从 CPU/GPU 整机结果中单独归因。
...
NPU 创建路径包含一个具体修正。Gemma3n 的第 19 组 prefill KV cache 没有连接到模型算子，LiteRT 因而为它分配 host memory；源码注释明确指出，该 buffer 与在 NPU 上运行的 transformer 不相容。执行器改从 decode signature 创建这一组 K、V 缓冲，清零后替换原 buffer。
...
LiteRT-LM 的 `BenchmarkInfo` 记录 executor 初始化、整次 prefill、整次 decode 与 TTFT。prefill/decode 计时从 turn start 到 turn end，适合作为端到端基线，但它不自动拆分 kernel、等待、采样和复制。要定位边界成本，需要在同一设备、同一模型和同一输入上做成对实验，并且每次只改变一个路径条件。
...
主 transformer 的编译选项允许 NPU 与 CPU，embedder 则显式采用 CPU 选项。`NpuConfig` 还分别控制 mask、KV cache update、per-layer embedding 与 greedy sampling 的实现选择，构造函数根据这些配置切换 mask 更新方式、KV cache 更新方式和 per-layer lookup 路径。所以，NPU executor 的性能取决于多段计算和交接，不等于一个 transformer NPU kernel 的耗时。

logits 路径也不同。主 NPU 模型的 decode 输出可以带 per-tensor 量化参数，创建执行器时读取 scale 与 zero point。需要返回 logits 时，执行器创建 host FP32 buffer，并调用 `DequantizeLogits` 写入该 buffer；常规内部 decode 则直接在 NPU 输出 buffer 上做 greedy 采样，并可选择 NEON 实现。这两条路径不能用同一份“logits 回传成本”估算。
...
该模型包名标注目标为 sm8750，记录中识别为 HTP V79 代；第一台设备为 HTP V81。这个静态差异说明预编译 context 还存在架构兼容条件，但执行在验证该条件前已经停止。本次测试没有直接观察到 V79/V81 不匹配引起的运行时错误，因此不能把它列为真机确认的另一个失败原因。

第二台设备是 HONOR MEP-AN00，系统库显示 HTP V79，且 `/odm` 中的 QNN 库可由 shell 读取。V79 代与模型包标注的目标代际更接近，但仅凭库名不能证明具体 SoC 与预编译 context 完全匹配。LiteRT-LM 先因宿主库版本检查失败；换用 QAIRT 2.46 宿主库后，流程进入 QNN manager 创建 backend/device 的阶段并在此停止，仍未创建可执行推理会话。`qnn-platform-validator` 使用原厂路径和 QAIRT 2.42 V79 组件时都报告相同的 unsigned-images 诊断。这个对照降低了“单一宿主 SDK 版本错配”作为唯一原因的可能性，并支持签名或 DSP 访问策略相关的解释；它不足以证明所有版本交互均已排除，也不足以把失败唯一归因于 OEM ROM。

子模型通过重复的 `TensorBuffer` 句柄衔接。prefill 路径对主模型 embeddings 输入缓冲调用 `Duplicate()`，并把所得句柄作为 embedder 输出；decode、verify、per-layer embedder 与 mask 路径采用同类连接。代码在这些连接点没有显式执行字节复制，`Duplicate()` 共享底层缓冲的句柄语义可以减少子图之间的显式复制。本书没有测量实际驱动同步与设备内存行为，因此不能把衔接成本记为零。
...
当两个候选 token 的 logits 接近时，微小数值差异可能改变 argmax 或采样结果。某一步选择不同 token 后，两次运行的后续上下文和计算输入也随之不同。不同硬件路径不保证逐比特一致，但输出差异也不能自动视为正常。排查时应固定模型、prompt、后端配置、采样参数、随机种子和线程设置，再比较 logits 与中间张量。按第 5 章核验的采样器实现，greedy 由 `k == 1` 决定，单独把温度设为 0 不等于 greedy。若排除随机采样后差异仍在，还需检查数值容差、算子实现和确定性设置。
...
后端 A/B 还要显式固定 sampler 后端。未指定 sampler 后端时，采样器沿用主后端：主后端为 GPU 就用 GPU sampler，为 CPU 就用 CPU sampler。因此，单独切换 `--backend=cpu/gpu` 会同时改变主模型与 sampler，不能把差异只归因于矩阵 kernel。诊断配置应显式指定 sampler，并关闭 MTP、约束解码与重复惩罚等会修改 logits 或执行步数的功能。
...
3. 调用不采样的 `DecodeLogits`，把两份 logits 转换为同一种 host 类型后比较。接口在执行器基类中定义，输出形状为 `[batch, sequence, vocab]`；具体 compiled executor 返回当前输出 buffer 的句柄。
4. 同时计算最大绝对误差、相对误差与 top-2 margin。若误差远小于 margin，top-1 理应稳定；若二者同量级，token 选择可能改变。再按算子或子图边界逐层比较，定位首个超出容差的位置。

给一个不代表实测结果的数值例子。CPU 路径中，候选 A、B 的 logits 分别为 12.002 和 12.000，top-2 margin 为 0.002。GPU 路径若得到 11.998 和 12.001，B 会成为 top-1。四个值的最大跨后端差为 0.004，大于 CPU 路径的 margin，最终文本可能从这一 token 起完全不同。诊断结论只能是“首个分叉处的数值扰动足以改变 argmax”，不能据此判定整条 GPU 路径错误。
...
`num_logits_to_print_after_decode` 只能打印 logits 开头、中间和末尾的若干值，top-2 token 不一定位于这些区间内。完整诊断更适合由测试 harness 消费 `DecodeLogits`，把全量 logits 保存为二进制或计算摘要；开启日志时还要排除上一节说明的回读开销。

端侧实验记录必须包含后端及其配置。附录 D 因此把不同后端作为不同实验条件，不合并比较。

MoE 会进一步改变后端的分派与矩阵形状。10.6 节对照 CPU 的逐专家执行和 GPU 的专家分组，说明相同算子名称下的布局、支持范围与激活函数仍需分别核对。

## 8.8　带壳手机的持续运行案例

短时测试给出的吞吐，还要放到实际散热条件下观察。附录 D 第十四节在 HONOR MEP-AN00 上使用本书基准所用的 Gemma 4 E4B 文件，固定 GPU OpenCL 主干与设备侧采样；手机带日常使用的手机壳，没有风扇或散热器，未开启性能模式，USB 保持连接，室温由作者估计为 22–25 °C。

固定负载的输入为 77 token，上下文上限 4096，MTP 关闭，本组开启 500 ms 的进程内存采集。运行持续 901.7 s，44 个独立会话的每次 decode 计数均为 201 token，全部正常结束，44 次输出内容一致。第 2、3 个请求的 decode 吞吐为 17.85、17.71 tokens/s，第 5 个请求已降至 9.71 tokens/s；按请求开始时刻归组，最后 120 s 内的 5 次请求汇总为 9.73 tokens/s。不能把开头接近 18 tokens/s 的值当作持续运行的预算。

在这 901.7 s 内，系统热状态由轻度转为中度，机身表面（skin）温度传感器的读数从 38.064 °C 升至 42.725 °C，时间曲线与完整条件见附录 D 图 D-1。这里记录到的是同一负载下吞吐与热状态的共同变化，尚未测出频率、同步、访存和各执行单元各自贡献了多少下降；结束时机身表面温度仍在上升，15 分钟的记录也不等于已经测到热稳态。

对部署而言，除了确认后端能够完整生成，还需要为持续运行单独设定验收条件：固定输入与会话策略，记录温度和系统热状态，并在开始前确定时长及停止条件。本例没有可靠的功率数据，不能比较 CPU、GPU 与 NPU 的能效；它支持的结论只有一条：这台带壳手机上，该固定负载的持续吞吐低于前几轮的吞吐。
