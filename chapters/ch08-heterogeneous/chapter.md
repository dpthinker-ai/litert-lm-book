...
短时测试给出的吞吐，还要放到实际散热条件下观察。附录 D 第十四节在 HONOR MEP-AN00 上使用同一份 Gemma 4 E4B 文件，固定 GPU OpenCL 主干与设备侧采样。手机带日常使用的手机壳，没有风扇或散热器，未开启性能模式；USB 保持连接，室温由作者估计为 22–25 °C。

固定负载的输入为 77 token，上下文上限 4096，MTP 关闭。44 个独立会话的每次 decode 计数均为 201 token。运行持续 901.7 s，全部正常结束，输出内容也一致。本组开启 500 ms 进程内存采集。第 2、3 个请求的 decode 吞吐为 17.85、17.71 tokens/s，第 5 个请求已降至 9.71 tokens/s。按请求开始时刻归组，最后 120 s 内的 5 次请求汇总为 9.73 tokens/s。不能把开头接近 18 tokens/s 的值作为持续运行预算。

在这 901.7 s 内，系统热状态由轻度转为中度，skin 传感器读数从 38.064 °C 升至 42.725 °C。时间曲线与完整条件见附录 D 图 D-1。这里记录到的是同一负载下吞吐与热状态的共同变化，尚未测出频率、同步、访存和各执行单元分别贡献了多少下降。结束时 skin 温度仍在上升，因此 15 分钟的记录也不等于已经测到热稳态。

对部署而言，除了确认后端能够完整生成，还需要给持续运行保留独立的验收条件。应固定输入与会话策略，记录温度和系统热状态，并在开始前确定时长及停止条件。本例没有可靠功率数据，不能比较 CPU、GPU 与 NPU 的能效。它支持的结论是，这台带壳手机上该固定负载的持续吞吐低于前几轮的吞吐。
